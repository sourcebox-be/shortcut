<script lang="ts">
    import logo from '../assets/shortcut.svg'
    ...
    let hashIds = new Hashids(Date.now().toString(), 4);

    let url, code, copied = false;

    const onSubmit = async() => {
        const { data, error } = await supabase
            .from('shortcuts')
            .insert({ url: url, short_code:  hashIds.encode(Date.now())})
            .select()
    ...
    <div class="hero-content text-center">
      <div class="max-w-md">
        <div class="flex flex-col items-center justify-center">
          <img src={logo} class="logo" alt="Shortcut by Sourcebox">
        </div>
